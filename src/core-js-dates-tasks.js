...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (
    (new Date(dateEnd).getTime() - new Date(dateStart).getTime() + secInDay) /
    secInDay
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  return daysArr.filter((date) => date.getDay() === 6 || date.getDay() === 0)
    .length;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysDiff = (date - startYear) / (1000 * 60 * 60 * 24);
...
function getNextFridayThe13th(date) {
  const lastDayInYear = new Date(date.getFullYear(), 12, 0);
  const daysDiff = (lastDayInYear - date) / (1000 * 60 * 60 * 24);
  const daysArr = [];
  for (let i = 1; i <= daysDiff; i += 1) {
    daysArr.push(new Date(date.getFullYear(), 0, i));
  }
  return daysArr.filter((day) => day.getDay() === 5 && day.getDate() === 13)[0];
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const workingDays = daysArr.filter(
    (day, index) =>
      (index + 1) % (countWorkDays + countOffDays) <= countWorkDays &&
      (index + 1) % (countWorkDays + countOffDays) > 0
  );
  return workingDays.map((day) =>
    day.toISOString().slice(0, 10).split('-').reverse().join('-')
  );
}
